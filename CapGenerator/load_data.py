...
def load_set(filename):
    ...
    for line in doc.split('\n'):
        ...
        #print('sample load_data {}'.format(dataset[:5]))
    return set(dataset)
...
def prepare_dataset(data='dev'):
    ...
    if data == 'dev':
        ...
        # descriptions
        train_descriptions = load_clean_descriptions('../att_model/descriptions.txt', train)
        test_descriptions = load_clean_descriptions('../att_model/descriptions.txt', test)
        print('Descriptions: train=%d, test=%d' % (len(train_descriptions), len(test_descriptions)))

        # photo features
        train_features = load_photo_features('../att_model/features.pkl', train)
        test_features = load_photo_features('../att_model/features.pkl', test)
